...
Signing runs use the `release-sign` policy. Because that policy requires manual
approval, each signing job waits up to 5 hours 45 minutes for approval and
completion. Approve both requests in order: the MSI request first, followed by
the setup-EXE request after the signed MSI has been embedded.
