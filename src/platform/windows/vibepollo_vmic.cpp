...
namespace platf::audio {
  vibepollo_vmic_t::~vibepollo_vmic_t() = default;

  std::string_view vibepollo_vmic_t::backend_id() const {
  ...
  int vibepollo_vmic_t::write_data(const char *data, std::size_t len, std::uint16_t sequence_number, std::uint32_t timestamp) {
    if (!speaker_backend) {
