...
namespace VDISPLAY {
  ...
  std::vector<SudaVDADisplayInfo> enumerateSudaVDADisplays() {
    ...
    const std::wstring sudoMakerDeviceString = L"SudoMaker Virtual Display Adapter";
    DISPLAY_DEVICEW displayDevice;
    displayDevice.cb = sizeof(DISPLAY_DEVICE);

    int deviceIndex = 0;
    while (EnumDisplayDevicesW(NULL, deviceIndex, &displayDevice, 0)) {
      if (std::wstring(displayDevice.DeviceString) == sudoMakerDeviceString) {
        SudaVDADisplayInfo info;
        info.device_name = displayDevice.DeviceName;
        info.friendly_name = displayDevice.DeviceString;
        info.is_active = (displayDevice.StateFlags & DISPLAY_DEVICE_ACTIVE) != 0;
        info.width = 0;
        info.height = 0;
