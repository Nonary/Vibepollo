      ...
      if (hdr_profile_utf8 && !hdr_profile_utf8->empty()) {
        ...
        if (!profile_path) {
          BOOST_LOG(warning) << "HDR profile: configured profile '" << *hdr_profile_utf8 << "' not found in '"
                             << platf::to_utf8(default_color_profile_directory().wstring()) << "' for client '" << *client_name_utf8
                             << "'.";
        }
      }
      if (!profile_path) {
        profile_path = find_hdr_profile_for_client(*client_name_utf8);
      }
      if (!profile_path) {
        BOOST_LOG(debug) << "HDR profile: no matching profile found for client '" << *client_name_utf8
                         << "' in '" << platf::to_utf8(default_color_profile_directory().wstring()) << "'.";
      }
