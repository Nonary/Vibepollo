...
namespace webrtc_stream {
  ...
  namespace {
    ...
    void handle_input_message(std::string_view payload) {
      ...
      constexpr auto permission = crypto::PERM::_all_inputs;
      ...
      if (type == "gamepad_connect") {
        ...
        input::passthrough(input_ctx, make_gamepad_arrival_packet(controller, controller_type, capabilities, supported_buttons), permission);
        return;
      }
    ...
    std::optional<std::string> start_webrtc_capture(const SessionOptions &options) {
      ...
      if (requested_app_id > 0 && requested_app_id != current_app_id) {
        ...
        auto it = std::find_if(apps.begin(), apps.end(), [&](const proc::ctx_t &app) {
          return util::from_view(app.id) == requested_app_id;
        });
        if (it == apps.end()) {
          return std::string {"Unknown application id "} + std::to_string(requested_app_id);
        }

        auto result = proc::proc.execute(*it, launch_session);
        if (result != 0) {
    ...
    void handle_input_message_binary(SessionDataChannelContext *ctx, const std::uint8_t *buffer, std::size_t length) {
      ...
      input::passthrough(input_ctx, make_abs_mouse_move_packet(unit_from_u16(x_u16), unit_from_u16(y_u16)), permission);
    }
        ...
        lwrtc_encoded_video_source_set_keyframe_callback(
            ...
            [](void* user) {
              ...
              if (mail) {
                ...
                if (idr_events) {
                  idr_events->raise(true);
                  BOOST_LOG(debug) << "WebRTC: keyframe requested via PLI/FIR";
                }
              }
            },
            nullptr);
