  ...
  internal static class InstallerRunner {
    ...
    public static InstallerResult RunInteractiveInstall(
      ...
      if (exitCode == 0 || exitCode == 3010) {
        if (installVirtualDisplayDriver && !IsSudoVdaDevicePresent()) {
          componentFailures.Add("SudoVDA virtual display driver failed to install.");
        }
      }
    ...
    private static bool IsSudoVdaDevicePresent() {
      return HasRegistrySubkeys(@"SYSTEM\CurrentControlSet\Enum\ROOT\SUDOMAKER\SUDOVDA")
        || HasRegistrySubkeys(@"SYSTEM\CurrentControlSet\Enum\ROOT\SUDOVDA");
    }

    private static bool HasRegistrySubkeys(string keyPath) {
